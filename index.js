...
const fetch = require("node-fetch"); // Important for Node < 18
...
const ENTRATA_USERNAME = process.env.ENTRATA_USERNAME; // Render env var
const ENTRATA_PASSWORD = process.env.ENTRATA_PASSWORD; // Render env var
...
// ── Debug endpoint to test Sleeknote mapping ─────────────────────────
...
app.post("/", async (req, res) => {
  ...
  const moveInDate = pick(raw.moveInDate, raw["Move In Date"]);
  const notes = pick(raw.notes, raw.message, raw["Message"]);
